<template>
  <b-container fluid>
    <b-row v-for="skill in skills" :key="skill.label" align-h="start">
      ...
      <b-col>
          <label :id="skill.label" :class="skill.value >= skill.orgValue ? skill.value === skill.orgValue ? '' :'positive' : 'negative'">{{skill.value}} <font-awesome-icon v-if="skill.mods.length" icon="info" /></label>
