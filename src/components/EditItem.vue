<template>
  <div>
      ...
      <div>
        <b-form-select class="mb-3">
          <b-form-select-option
            v-model="invTransIndex"
            v-for="(inv, index) in getChar.inventory"
            :key="index"
            :value="index"
            >{{ inv.name }}</b-form-select-option
          >
        </b-form-select>

        <button @click="move(invTransIndex)">
          <h5>In inventar transferieren</h5>
        </button>
      </div>
    ...
    <div v-for="(modGroup, index) in mods" :key="index">
      <p v-if="active" class="left" v-b-toggle="index" variant="primary">
        -{{ index }}
      </p>
    ...
    <button :disabled="!name || !slot" @click="deleteItem()">Löschenn</button>
  </div>
</template>
<script>
...
export default {
  ...
  mounted () {
    if (this.item) {
      ...
      this.mods = {
        ...JSON.parse(JSON.stringify(this.getItem.modifiers)),
        ...this.item.modifiers
      }
    } else {
  ...
  methods: {
    ...
    equip () {
      ...
      this.getChar.equipped[this.item.slot] = Object.assign({}, this.item)
      if (oldItem) {
    ...
    deleteItem () {
      const itemIndex = this.inventory.items.indexOf(
        this.inventory.items.find(element => element.name === this.item.name)
      )
      this.inventory.items.splice(itemIndex, 1)
    },
    move (index) {
      this.getChar.inventory[index].items.push(Object.assign({}, this.item))
      if (this.equipped) {
      ...
      for (const index in this.mods) {
        for (const stat in this.mods[index]) {
          if (!(this.mods[index][stat].mod + this.mods[index][stat].multi)) {
            ...
            this.mods[index][stat].multi =
              parseFloat(this.mods[index][stat].multi) / 100
          }
        }
